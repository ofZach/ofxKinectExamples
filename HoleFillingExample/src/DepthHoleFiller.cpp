...
//==============================================================
void DepthHoleFiller::fillBlobsWithInterpolatedData (ofxCvGrayscaleImage &depthImage, int maxLevelDelta, int maxHoleFillWidth){
	
	// interpolates between one edge of the hole and the other
	// uses a scanline technique, and a simple nonlinear interpolation biased towards the low edges of holes.
		...
		for (int x=1; x<KW; x++){
			...
			if (bRunStarted){
				if ((bval0 == HOLE) && (bval1 != HOLE)){
					...
					if ((abs(vallen) <= maxLevelDelta) && (runlen < maxHoleFillWidth)){
						...
						if (runlen <= 1){
							...
							bool bUseLowestValue = true;
							if (bUseLowestValue){
								unsigned char val = (runValueA < runValueB) ? runValueA: runValueB;
								for (int i=runIndexA; i<=runIndexB; i++){
									blobsPixels[i] = val;
								}
							} else {
								// smooth interpolation instead
								for (int i=runIndexA; i<=runIndexB; i++){
									float tf = (float)(i-runIndexA)/runlenf;
									tf = tf*tf*tf*tf*tf; // 5th power nonlinearity, why not
...
/*
//--------------------------------------------------------------
void DepthHoleFiller::fillHolesAlternate (ofxCvGrayscaleImage ofxCv8uC1_DepthRawThreshed) {
