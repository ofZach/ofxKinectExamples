...
DatumHistorian::DatumHistorian(){
	historyLength = 0;
	maxHistoryLength = 64;
	history 		= new float[maxHistoryLength]; //ippsMalloc_32f(maxHistoryLength); 
	sortedValues 	= new float[maxHistoryLength]; //ippsMalloc_32f(maxHistoryLength); 
	tempValues		= new float[maxHistoryLength]; //ippsMalloc_32f(maxHistoryLength); 
	clear();
}
//--------------------------------------------------------------
...
float DatumHistorian::getStandardDeviation(){
	...
	float out = 0;
	/*
	// Not working at the moment, due to disinclusion of Ipp. 
	if (historyLength > 2){
		Ipp32f pStdDev = 0;
...
//--------------------------------------------------------------
void DatumHistorian::resetHistory (float newval){
	for (int i=0; i<maxHistoryLength; i++){
		history[i] = newval;
	}
	runningAverage = newval;
	currentMedian  = newval;
}
